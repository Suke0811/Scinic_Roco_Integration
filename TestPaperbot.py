...
a.makeOutput("output/paperbot", thickness=5, tree=False, display=False, Webots=True, Dimensions=dim)
...
#             a=None
#             a = getComponent("Paperbot", length=length, width=width, height=height)
#             a.makeOutput("output/Paperbot/%(length)d_%(width)d_%(height)d/" %locals(), thickness=5, tree=False, display=False, Webots=False)
